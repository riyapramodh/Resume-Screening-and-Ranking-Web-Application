...
def clean_text(text, section_name):
    ...
    filtered_words = [word for word in words if word.lower() not in stop_words or word.lower() in retain_words]
...
def extract_data(resume_text):
    sections = re.split(r'\s*(EDUCATION|TECHNICAL\s+INTERESTS|PROJECTS|INTERNSHIP|TECHNICAL\s+SKILLS|CERTIFICATIONS|ACHIEVEMENTS\s+&\s+HONOURS|LANGUAGES|HOBBIES)\s*', resume_text)
    data = {}
    ...
    for i in range(1, len(sections), 2):
        ...
        if section_name in ['EDUCATION', 'TECHNICAL INTERESTS', 'PROJECTS', 'INTERNSHIP', 'TECHNICAL SKILLS', 'CERTIFICATIONS', 'ACHIEVEMENTS & HONOURS', 'LANGUAGES', 'HOBBIES']:
...
def process_resume_data(recruiter_id):
    ...
    pdf_files = list(db.fs.find({'recruiter_id': recruiter_id}))

    # Download PDFs to the temporary folder
    for pdf_file in pdf_files:
        temp_file_path = os.path.join(temp_folder, pdf_file.filename)
        with open(temp_file_path, 'wb') as temp_file:
            temp_file.write(db.fs.get(pdf_file._id).read())

    # Process each resume PDF in the temporary folder
    collection = db.db['temp']  # Replace 'your_collection' with your collection name
        ...
        if pdf_file_path.endswith(".pdf"):
            with open(os.path.join(temp_folder, pdf_file_path), 'rb') as file:
                ...
                collection.insert_one({
                    ...
                    'AchievementsHonors': resume_data.get('ACHIEVEMENTS & HONOURS', ''),
